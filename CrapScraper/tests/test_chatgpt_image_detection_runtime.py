...
from app.additions.chatgpt_playwright import _image_candidate_key, _is_probable_generated_image
...
def test_generated_image_rejects_avatar_and_small_icon():
    assert not _is_probable_generated_image(
        {"src": "https://example.com/avatar.png", "width": 512, "height": 512, "alt": "avatar", "scope": "main", "index": 1}
    )
    assert not _is_probable_generated_image(
        {"src": "https://example.com/icon.png", "width": 64, "height": 64, "alt": "", "scope": "main", "index": 2}
    )


def test_candidate_key_distinguishes_duplicate_src_inserted_later():
    first = {"src": "blob:https://chatgpt.com/abc", "width": 1024, "height": 1024, "alt": "", "scope": "main", "index": 3}
