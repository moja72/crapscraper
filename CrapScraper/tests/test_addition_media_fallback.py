...
def test_media_auth_errors_allow_woocommerce_fallback():
    ...
    assert AdditionStoreGateway.media_upload_fallback_allowed(RuntimeError("Credenciais WordPress não configuradas")) is True
    response.status_code = 500
    assert AdditionStoreGateway.media_upload_fallback_allowed(error) is False
...
def test_publisher_can_publish_image_with_stable_public_url(tmp_path: Path, monkeypatch):
    ...
    assert url == "https://plugintema.com.br/downloads/add-demo-product-image.png"
    assert (target / "add-demo-product-image.png").read_bytes() == b"png"
