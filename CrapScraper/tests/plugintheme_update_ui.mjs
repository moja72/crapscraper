...
const payload=validated=>({ok:true,checks:checks(validated),attention_count:validated?0:1,plugintheme:{configured:true,account_key:"account-a",profile_exists:true,persistence_mode:"persistent_browser_context",cookie_count:validated?7:5,httponly_cookie_count:validated?1:0,storage_entry_count:validated?9:0,current_url:validated?"https://plugintheme.net/pt-BR/account":"",login_redirect:false,authenticated:validated,status:validated?"VALIDADA":"CONFIGURADA / SESSÃO NÃO VALIDADA",renewal_available:true,credits:validated?12:null,credit_limit:validated?50:null,credit_status:validated?"success":"unavailable",credit_stale:false,credit_source:validated?"api:quota":"",last_error:validated?"":"Validação pendente.",logs:validated?["Sessão autenticada confirmada.","Consultando créditos...","Créditos disponíveis: 12."]:[]}});
...
try{
  ...
  if(!result.source.includes("VALIDADA")||!result.session.includes("VALIDADA")||!result.session.includes("7 cookie")||!result.session.includes("HttpOnly")||!result.credit.includes("12")||!result.status.includes("Sessão autenticada confirmada")||!result.status.includes("Limite diário do plano: 50")||result.minHeight<40||checksCalled!==1)throw new Error(`Estado validado incorreto: ${JSON.stringify({result,checksCalled})}`);
