...
def test_materialize_accepts_integral_legacy_woo_id(tmp_path):
    row=approval();row["woo_product_id"]="90109.0"
    repo=UpdateRepository(tmp_path);repo.materialize([row])
    assert repo.list()["items"][0]["woo_product_id"]==90109
