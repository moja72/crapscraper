...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if($name -notin @('python','pythonw') -or $command -notmatch 'main\.py' -or $command -notmatch $repo){Write-Host ('[CrapScraper] A porta ' + $port + ' pertence a outro processo; nada foi encerrado. PID ' + $serverPid); exit 2};" ^
