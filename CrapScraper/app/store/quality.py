...
class StoreQualityService:
    def inspect(self,products,gateway):
        issues=[]
        ...
        for product in products:
            ...
            if str(product.get("type"))=="variable":
                variations=gateway.variations(product["id"]);periods={" ".join(str(a.get("option") or "").lower().split()) for v in variations for a in v.get("attributes",[]) or []}
                if not any("anual" in x for x in periods) or not any("vital" in x for x in periods):add(product,"invalid_variations","variations","Variações Anual/Vitalícia incompletas.","error")
