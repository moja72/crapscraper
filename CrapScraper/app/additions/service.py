...
class AdditionService:
    def __init__(self, data_dir: Path, *, repository=None, executor=None):
        self.repository = repository or AdditionRepository(data_dir)
        self.executor = executor or AdditionExecutor(self.repository)
        self.batch = AdditionBatchService(self.executor)
        ...
        # para diagnóstico/migração controlada com SCRAPER_ADDITION_IMPORT_LEGACY=1.
        if os.getenv("SCRAPER_ADDITION_IMPORT_LEGACY", "0").lower() not in {"0", "false", "no", "off"}:
            self.repository.migrate_legacy(data_dir / "addition_jobs.sqlite3")

        # A fila normal começa vazia. Aprovações só entram quando o usuário clica
        # em "Sincronizar aprovados" ou quando uma adição manual é materializada.
        if os.getenv("SCRAPER_ADDITION_E2E_FIXTURES", "") == "1":
    ...
    def _seed_e2e_fixtures(self):
        approvals = [
            {
                "comparison_item_id": f"e2e-add-{number:02d}",
                "source_name": "PluginTheme",
                "source_version": "2.0",
                "source_product_url": f"https://plugintheme.net/item/e2e-{number:02d}",
                "source_official_url": "https://developer.example/product",
            }
            for number in range(12)
        ]
        self.repository.materialize(approvals)
        items = self.repository.list(page_size=100)["items"]
        if items:
            self.repository.patch(
                items[0]["job_id"],
                public_state="error",
                stage="fixture_error",
                current_error={"message": "Erro simulado E2E", "code": "fixture_error"},
            )
        if len(items) > 1:
            self.repository.patch(items[1]["job_id"], public_state="success", stage="completed")

    def materialize(self):
        with self.lock:
            return {"ok": True, **self.repository.materialize(decisions.list_approved_additions())}

    def materialize_manual(self, approval):
        with self.lock:
            self.repository.materialize([approval])
            job_id = self.repository.job_id(str(approval["comparison_item_id"]))
    ...
    def batch_start(self, ids=None):
        return {
            "ok": True,
            "batch": self.batch.start(
                ids or [x["job_id"] for x in self.repository.list(group="prepared", page_size=100)["items"]]
            ),
        }

    def batch_control(self, action):
        return {"ok": True, "batch": self.batch.control(action)}
