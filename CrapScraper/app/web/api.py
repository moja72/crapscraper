...
class ApplicationServices:
    ...
    def build(cls, settings: Settings, runtime: JsonStore) -> "ApplicationServices":
        credits=CreditService(settings.data_dir);updates=UpdateService(settings.data_dir,credits=credits);comparison=ComparisonService(settings.data_dir);additions=AdditionService(settings.data_dir)
        ...
        return cls(CollectionService(settings.data_dir), comparison, updates, additions, store, ManualSyncService(comparison,updates,additions), CatalogService(settings.data_dir,store.gateway), credits)
