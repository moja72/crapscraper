...
from app.updates.adapters import Installer, WooCommerceGateway, WooGateway, build_installer, product_version
...
class UpdateExecutor:
    ...
    def execute(self, job_id: str) -> dict[str,Any]:
        ...
        backup=None;original_version="";critical=False;artifact_sha="";stage="validating"
        ...
        try:
            ...
            if original_version==job["source_version"]:
                progress("already_current",f"Destino já está na versão {original_version}; nenhuma escrita necessária.")
            ...
            progress("staging",f"ZIP validado ({artifact.size} bytes, SHA-256 {artifact.sha256[:12]}…).")
            backup=self.installer.backup(job,attempt_dir);progress("backing_up","Backup do destino confirmado.")
            critical=True;progress("installing","Instalando artefato validado.");self.installer.install(job,artifact.path,backup)
            if not self.installer.validate(job,artifact.sha256): raise RuntimeError("Validação SHA-256 do destino falhou")
            progress("updating_woocommerce",f"Atualizando pt_versao para {confirmed}.");self.woo.set_version(int(job["woo_product_id"]),confirmed)
            ...
            if critical and backup is not None:
                try:
                    ...
                    if original_version: self.woo.set_version(int(job["woo_product_id"]),original_version)
