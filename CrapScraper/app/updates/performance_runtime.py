...
def _batch_start(self: UpdateService, job_ids: list[str] | None = None) -> dict[str, Any]:
    """Executa elegíveis e ignora seleção obsoleta em vez de travar o lote inteiro."""
    ...
    for job_id in requested:
        ...
        blockers_for_error.extend(
            {**item, "message": f"{job.get('product_name')}: {item.get('message')}"}
            for item in blockers
        )

    if not eligible:
        raise UpdateExecutionBlocked(blockers_for_error or [{"code": "no_eligible_jobs", "message": "Nenhum item selecionado está elegível para execução."}])
...
__all__ = ["install_update_performance_runtime"]
