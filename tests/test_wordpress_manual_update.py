...
class WordPressManualUpdateTests(unittest.TestCase):
    ...
    def test_wordpress_plugin_has_server_side_super_admin_and_ui_states(self):
        ...
        self.assertIn("crapscraper_manual_history", php)
        self.assertIn("status='completed' ORDER BY completed_at DESC, id DESC LIMIT 3", php)
        self.assertIn("'/update-history'", php)
        self.assertIn("UNIQUE KEY operation_id", php)
        self.assertIn("date_i18n('d/m/Y H:i'", php)
        self.assertIn("data-cs-history-toggle", php)
        self.assertIn("loadHistory", js)
        self.assertIn("slice(0, 3)", js)
        self.assertIn("'⬇️'", js)
        self.assertIn("width:56px;height:56px", css)
        self.assertIn("is-minimized>:not(.cs-panel-head)", css)
