...
class CollectionUiContractTests(unittest.TestCase):
    ...
    def test_comparison_can_generate_and_select_plugintema_catalog(self) -> None:
        ...
        self.assertNotIn('friendly_label = f"{mode_label} atualizados em', web)
        self.assertIn('friendly_label = mode_label.upper()', web)
        self.assertIn('data-catalog-action="download">⬇️ Baixar</button>', js)
        self.assertIn('button.dataset.catalogAction === "download"', js)
