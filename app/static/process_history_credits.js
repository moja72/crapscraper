(() => {
  ...
  function recordHistory(process) {
    ...
    const duplicate = history.find(item => item.processId === row.processId && Math.abs(Number(item.finishedAt || 0) - row.finishedAt) < 1200);
    if (duplicate) return;
    history.unshift(row);
  ...
  function historyCard(process) {
    ...
    return `<article class="cs-process-history-card">
      ...
      <div class="cs-process-history-times"><span><b>Início:</b> ${escapeHtml(formatDate(process.startedAt))}</span><span><b>Fim:</b> ${escapeHtml(formatDate(process.finishedAt))}</span></div>
    </article>`;
  }

  function historySignature() { return history.map(item => item.historyId).join("|"); }

  function historyMarkup() {
    const rows = history.slice(0, HISTORY_LIMIT);
    return `
      <div class="cs-process-history-head"><div class="cs-process-history-title">Processos concluídos</div><div class="cs-process-history-count">${rows.length} registro(s)</div></div>
      <div class="cs-process-history-list">${rows.length ? rows.map(historyCard).join("") : '<div class="cs-process-history-empty">Nenhum processo concluído registrado neste painel.</div>'}</div>`;
  }

  function decorateModal() {
    const body = $("#cs_processes_body");
    if (!body) return;
    const activeCards = Array.from(body.children).filter(node => node.matches?.(".cs-process-card:not(.is-recent)"));
    ...
    if (!activeCards.length && !nativeEmpty) {
      if (!activeEmpty) {
        ...
        if (existingSection) body.insertBefore(activeEmpty, existingSection); else body.appendChild(activeEmpty);
      }
    } else if (activeEmpty) activeEmpty.remove();
  ...
  function creditLabel(payload) {
    const remaining = Number(payload?.remaining), limit = Number(payload?.limit);
    if (payload?.ok && Number.isFinite(remaining) && Number.isFinite(limit) && limit >= 0) return `${Math.max(0, remaining)}/${Math.max(0, limit)}`;
    return "—";
  }
  ...
  function ensureCreditsNode() {
    const button = $("#cs_processes_button"), parent = button?.parentElement;
    if (!button || !parent) return null;
    let node = $("#cs_download_credits");
    if (!node) {
      node = document.createElement("div");
      node.id = "cs_download_credits";
      node.innerHTML = `<div id="cs_credit_ultrapack" class="is-loading">UltraPackV2: <b>—</b></div><div id="cs_credit_plugintheme" class="is-loading">PluginTheme: <b>—</b></div>`;
    }
    if (node.parentElement !== parent || node.previousElementSibling !== button) button.insertAdjacentElement("afterend", node);
  ...
  async function pollCredits() {
    ...
    try {
      ...
      lastCreditsPayload = {ultrapackv2:{ok:false, message:"Não foi possível consultar os créditos."}, plugintheme:{ok:false, message:"Não foi possível consultar os créditos."}};
      renderCredits(lastCreditsPayload);
    } finally { creditsLoading = false; }
  }
  ...
  function requestDefinition(path, method) {
    ...
    if (write && path.endsWith("/loja/produtos/campos-ausentes")) return {kind:"store", title:"Início de auditoria de qualidade da Loja"};
    ...
    if (write && path.endsWith("/adicoes/automatico")) return {kind:"addition", title:"Início do cadastro automático"};
  ...
  window.fetch = function processHistoryFetch(input, init = {}) {
    ...
    return upstreamFetch(input, init).then(response => {
      recordHistory({id, title:definition.title, kind:definition.kind, detail:path, status:response.ok ? "Concluído" : `Erro HTTP ${response.status}`, error:!response.ok, startedAt, finishedAt:Date.now()});
      return response;
    }, error => {
      recordHistory({id, title:definition.title, kind:definition.kind, detail:text(error?.message || path), status:"Erro", error:true, startedAt, finishedAt:Date.now()});
      throw error;
    });
  };
  ...
  function updateBackendMap(runtime, target) {
    const jobs = Array.isArray(runtime?.jobs) ? runtime.jobs : [], queue = runtime?.queue || {};
    ...
    if (text(queue.status) === "running" || executing) {
      target.set(`backend:update-queue:${activeName}`, {
        id:`backend:update-queue:${activeName}`, title:`Fila de atualização · ${activeName === "default" ? "Padrão" : activeName}`, kind:"update",
        ...
        startedAt:parsedTime(executing?.executing_at)
      });
    }
    listJobs.filter(job => PREPARING_STATES.has(text(job?.state))).forEach(job => {
      target.set(`backend:prepare:${job.job_id}`, {id:`backend:prepare:${job.job_id}`, title:"Preparação de atualização", kind:"update", detail:`${text(job.name)} · Woo #${text(job.woo_product_id)}`, startedAt:parsedTime(job.updated_at || job.created_at)});
    });
  }

  function storePriceMap(data, target) {
    if (text(data?.status) !== "running") return;
    target.set("backend:store-price", {id:"backend:store-price", title:"Atualização de preços da Loja", kind:"store", detail:text(data.current_product || data.message || "Atualizando preços"), startedAt:Date.now()});
  }

  function storeDescriptionMap(data, target) {
    if (text(data?.status) !== "running") return;
    target.set("backend:store-description", {id:"backend:store-description", title:"Verificação de descrições da Loja", kind:"store", detail:text(data.current_product || data.message || "Verificando produtos"), startedAt:Date.now()});
  }

  function storeQualityMap(data, target) {
    if (text(data?.status) !== "running") return;
    target.set("backend:store-quality", {id:"backend:store-quality", title:"Auditoria de qualidade da Loja", kind:"store", detail:text(data.current_product || data.message || "Verificando produtos"), startedAt:Date.now()});
  }

  function additionAutomaticMap(data, target) {
    const tasks = Array.isArray(data?.tasks) ? data.tasks : [];
    tasks.filter(task => task?.running === true).forEach(task => {
      const jobId = text(task?.job_id || task?.job?.job_id || "addition"), job = task?.job || {};
      target.set(`backend:addition-auto:${jobId}`, {id:`backend:addition-auto:${jobId}`, title:"Cadastro automático de produto", kind:"addition", detail:text(job?.title || job?.source_name || jobId), startedAt:parsedTime(task?.started_at)});
    });
  }

  function wordpressManualMap(data, target) {
    const status = text(data?.monitor_status).toLowerCase(), requestId = text(data?.request_id);
    if (!requestId || !MANUAL_ACTIVE_STATES.has(status)) return;
    const productId = Number(data?.product_id || 0);
    target.set(`backend:wordpress-manual:${requestId}`, {id:`backend:wordpress-manual:${requestId}`, title:"Atualização solicitada pelo WordPress", kind:"update", detail:text(data?.product || (productId ? `Woo #${productId}` : requestId)), startedAt:parsedTime(data?.last_check)});
  }
  ...
  function collectionMap(data, target) {
    ...
    runs.filter(run => run?.running === true || /rodando|running|iniciando|processando/i.test(text(run?.status))).forEach(run => {
      ...
      target.set(`backend:run:${runId}`, {id:`backend:run:${runId}`, title:"Coleta de catálogo", kind:"collection", detail:text(run.summary || [run?.context?.site_key, run?.context?.item_type_key, run?.context?.slot_name].filter(Boolean).join(" · ") || runId), startedAt:Date.now() - Math.max(0, Number(run.timer_seconds || 0) * 1000)});
    });
  }
  ...
  function syncBackend(next) {
    ...
    for (const [id, previous] of backendSeen) if (!next.has(id)) recordHistory({...previous, status:"Finalizado", error:false, finishedAt:Date.now()});
    backendSeen = next;
  }
  ...
  async function pollBackendHistory() {
    ...
    if (results[4].status === "fulfilled") storeQualityMap(results[4].value, next); else preservePrefix(next, "backend:store-quality");
    if (results[5].status === "fulfilled") additionAutomaticMap(results[5].value, next); else preservePrefix(next, "backend:addition-auto:");
    if (results[6].status === "fulfilled") wordpressManualMap(results[6].value, next); else preservePrefix(next, "backend:wordpress-manual:");
    syncBackend(next);
  }

  function observeUi() {
    const observer = new MutationObserver(() => { ensureCreditsNode(); decorateModal(); });
