...
def _failure_summary(error: Any) -> str:
    ...
    if "autenticação do ultrapackv2 não configurada" in lowered or "autenticacao do ultrapackv2 nao configurada" in lowered:
        return "A conta do UltraPackV2 não está autenticada no CrapScraper; configure ou restaure a sessão antes de tentar novamente."
    if "autenticação do plugintheme não configurada" in lowered or "autenticacao do plugintheme nao configurada" in lowered:
        return "A conta do PluginTheme não está autenticada no CrapScraper; configure ou restaure a sessão antes de tentar novamente."
...
def _patched_job_public(job: OperationalJob) -> dict[str, Any]:
    ...
    data["status_at"] = _clean(job.completed_at if job.state == JobState.COMPLETED else job.updated_at)
    return data
...
__all__ = ["install_update_reliability_observability_policy", "_failure_summary"]
