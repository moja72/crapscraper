...
Versão 2.5.0: a sanfona mostra somente as três atualizações realmente concluídas mais recentes do produto, com data, origem e versões. Sucessos do executor canônico são recebidos pelo endpoint HMAC de histórico, gravados de forma idempotente por `operation_id` e confirmados por leitura. Pedidos manuais existentes são reutilizados quando representam a mesma operação, sem duplicar o registro. Quando minimizado, o painel exibe somente o ícone `⬇️`. Movimento, persistência local e todas as proteções do fluxo seguro permanecem ativos.
