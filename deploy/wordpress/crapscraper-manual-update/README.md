...
Versão 2.6.0: o protocolo distingue verificação, atualização encontrada, execução, conclusão, no-op (`already_updated`) e falha. A sanfona mostra as três operações terminais mais recentes do produto, com data, origem e versões. Sucessos do executor canônico são recebidos pelo endpoint HMAC de histórico, gravados de forma idempotente por `operation_id` e confirmados por leitura. Pedidos manuais existentes são reutilizados somente quando representam a mesma solicitação, e leases interrompidos são retomados pelo monitor. Quando minimizado, o painel exibe somente o ícone `⬇️`. Movimento, persistência local e todas as proteções do fluxo seguro permanecem ativos.
