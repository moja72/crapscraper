<?php
/**
 * Plugin Name: CrapScraper WooCommerce Bridge
 * Description: Bridge HMAC restrito para operações de adição quando o WAF bloqueia /wc/v3.
 * Version: 1.0.0
 */
...
add_action('rest_api_init', static function (): void {
    register_rest_route('crapscraper/v1', '/store-command', [
        'methods' => 'POST',
        'callback' => 'crapscraper_store_bridge_command',
        'permission_callback' => 'crapscraper_store_bridge_authorize',
    ]);
});

function crapscraper_store_bridge_secret(): string {
...
function crapscraper_store_bridge_authorize(WP_REST_Request $request) {
    $secret = crapscraper_store_bridge_secret();
    if (strlen($secret) < 24) {
        return new WP_Error('crapscraper_bridge_secret_missing', 'Segredo HMAC do CrapScraper não configurado.', ['status' => 503]);
    }

    $timestamp = trim((string) $request->get_header('x-crapscraper-timestamp'));
    $signature = strtolower(trim((string) $request->get_header('x-crapscraper-signature')));
...
function crapscraper_store_bridge_response($result) {
...
function crapscraper_store_bridge_request(string $method, array $params, array $body): WP_REST_Request {
...
function crapscraper_store_bridge_command(WP_REST_Request $outer) {
    if (!class_exists('WooCommerce')) {
        return new WP_REST_Response(['ok' => false, 'message' => 'WooCommerce não está ativo.'], 503);
    }

    $payload = $outer->get_json_params();
    if (!is_array($payload)) {
        $payload = [];
    }
    ...
    try {
        if ($path === '/products') {
            ...
            $request = crapscraper_store_bridge_request($method, $params, $body);
            if ($method === 'GET') {
                return crapscraper_store_bridge_response($controller->get_items($request));
            }
            if ($method === 'POST') {
                return crapscraper_store_bridge_response($controller->create_item($request));
            }
        ...
        if (preg_match('~^/products/(\d+)$~', $path, $matches)) {
            ...
            if ($method === 'GET') {
                return crapscraper_store_bridge_response($controller->get_item($request));
            }
            if ($method === 'PUT') {
                return crapscraper_store_bridge_response($controller->update_item($request));
            }
